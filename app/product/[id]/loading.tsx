export default function ProductLoading() {
  return (
    <main className="max-w-7xl mx-auto px-6 md:px-10 py-8 md:py-14">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14">
        {/* Image skeleton */}
        <div className="space-y-3">
          <div className="aspect-[3/4] w-full bg-neutral-100 rounded animate-pulse" />
          <div className="flex gap-2">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="w-16 h-20 sm:w-20 sm:h-24 bg-neutral-100 rounded animate-pulse flex-shrink-0" />
            ))}
          </div>
        </div>

        {/* Details skeleton */}
        <div className="space-y-6">
          <div className="h-3 w-16 bg-neutral-100 rounded animate-pulse" />
          <div className="h-8 w-3/4 bg-neutral-100 rounded animate-pulse" />
          <div className="h-7 w-28 bg-neutral-100 rounded animate-pulse" />
          <div className="h-4 w-24 bg-neutral-100 rounded animate-pulse" />
          <div className="space-y-2">
            <div className="h-4 w-full bg-neutral-100 rounded animate-pulse" />
            <div className="h-4 w-5/6 bg-neutral-100 rounded animate-pulse" />
          </div>
          <div className="flex gap-2">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-10 w-14 bg-neutral-100 rounded-lg animate-pulse" />
            ))}
          </div>
          <div className="h-12 w-full bg-neutral-100 rounded-full animate-pulse" />
        </div>
      </div>
    </main>
  );
}
