...
- [ ] Disable/hide the Order button (or show "Out of Stock" state) when `product.in_stock === false`.
...
- [x] Smoke-test the live Vercel URL: home page loads, shop page loads with real products, product detail + Order button opens WhatsApp correctly, admin login works, admin can add/edit/toggle a product live.
- [x] Hand off the Vercel production URL to the client for DNS pointing (client's nephew handles domain registration/DNS — out of scope for the agent).

---

## Phase 6 — Customer Photo Gallery (UGC / Social Proof)

- [x] Create `gallery_images` table in Supabase schema (defined in `docs/SCHEMA.sql`) with RLS for public read, authenticated write/delete.
- [x] Create `gallery-images` public storage bucket in Supabase (defined in `docs/SCHEMA.sql`) with public read, authenticated upload/delete.
- [x] Add `GalleryImage` interface to `lib/types.ts`.
- [x] Update `components/admin/ImageUploader.tsx` with `bucketName` prop support (defaulting to `product-images`, selectable for `gallery-images`).
- [x] Add `createGalleryImageAction` and `deleteGalleryImageAction` Server Actions in `app/admin/actions.ts`.
- [x] Build public responsive masonry gallery page `app/gallery/page.tsx` with `components/GalleryMasonry.tsx` and full-screen lightbox modal.
- [x] Add `app/gallery/loading.tsx` skeleton loader.
- [x] Add "Gallery" links to Desktop Header, Mobile Drawer, and Footer.
